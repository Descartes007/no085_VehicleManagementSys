    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yuangongId" name="yuangongId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="事故登记唯一编号" prop="shigudengjiUuidNumber">
                       <el-input v-model="ruleForm.shigudengjiUuidNumber"
                                 placeholder="事故登记唯一编号" clearable  :readonly="ro.shigudengjiUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else>
                       <el-form-item class="input" label="事故登记唯一编号" prop="shigudengjiUuidNumber">
                           <el-input v-model="ruleForm.shigudengjiUuidNumber"
                                     placeholder="事故登记唯一编号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="事故名称" prop="shigudengjiName">
                       <el-input v-model="ruleForm.shigudengjiName"
                                 placeholder="事故名称" clearable  :readonly="ro.shigudengjiName"></el-input>
                   </el-form-item>
                   <div v-else>
                       <el-form-item class="input" label="事故名称" prop="shigudengjiName">
                           <el-input v-model="ruleForm.shigudengjiName"
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="事故类型" prop="shigudengjiTypes">
                        <el-select v-model="ruleForm.shigudengjiTypes" :disabled="ro.shigudengjiTypes" placeholder="请选择事故类型">
                            <el-option
                                v-for="(item,index) in shigudengjiTypesOptions"
                    ...
                    <div v-else>
                        <el-form-item class="input" label="事故类型" prop="shigudengjiValue">
                        <el-input v-model="ruleForm.shigudengjiValue"
                            placeholder="事故类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="责任方" prop="zerenTypes">
                        <el-select v-model="ruleForm.zerenTypes" :disabled="ro.zerenTypes" placeholder="请选择责任方">
                            <el-option
                                v-for="(item,index) in zerenTypesOptions"
                                v-bind:key="item.codeIndex"
                                :label="item.indexName"
                                :value="item.codeIndex">
                            </el-option>
                    ...
                    <div v-else>
                        <el-form-item class="input" label="责任方" prop="zerenValue">
                        <el-input v-model="ruleForm.zerenValue"
                            placeholder="责任方" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  label="事故详情" prop="shigudengjiContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.shigudengjiContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.shigudengjiContent" label="事故详情" prop="shigudengjiContent">
                            <span v-html="ruleForm.shigudengjiContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info'"  class="input" label="发生时间" prop="shigudengjiTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.shigudengjiTime"
                                type="datetime"
                                placeholder="发生时间"
                                :disabled="ro.shigudengjiTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else>
                        <el-form-item v-if="ruleForm.shigudengjiTime" label="发生时间" prop="shigudengjiTime">
                            <span v-html="ruleForm.shigudengjiTime"></span>
                        </el-form-item>
                    </div>
                </el-col>
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yonghuId: false,
                    yuangongId: false,
                    shigudengjiUuidNumber: false,
                    shigudengjiName: false,
                    shigudengjiTypes: false,
                    zerenTypes: false,
                    shigudengjiContent: false,
                    shigudengjiTime: false,
                    insertTime: false,
                },
                ruleForm: {
                    yonghuId: '',
                    yuangongId: '',
                    shigudengjiUuidNumber: new Date().getTime(),
                    shigudengjiName: '',
                    shigudengjiTypes: '',
                    zerenTypes: '',
                    shigudengjiContent: '',
                ...
                rules: {
                   ...
                   shigudengjiUuidNumber: [
                              { required: true, message: '事故登记唯一编号不能为空', trigger: 'blur' },
                          ],
                   shigudengjiName: [
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=shigudengji_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.shigudengjiTypesOptions = data.data.list;
                    }
                });
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=zeren_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.zerenTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`shigudengji/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.shigudengjiCrossAddOrUpdateFlag = false;
                                        this.parent.search();
